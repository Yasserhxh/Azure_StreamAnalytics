...
using ProtobufDeserializer.Model.GH;
...
namespace ProtobufDeserializer
{
    public sealed class PacketDecoder
    {
        ...
        public PacketDecoder(DataReader parser, string codecId)
        {
            ...
            if (codecId == "16") ParseAndDecodeCodec16Data();
            if (codecId == "142") ParseAndDecodeCodec8EData();
        }

        private void ParseAndDecodeCodec8EData()
        {
            ...
            ioElementSubItems.Add(_parser.ReadData(2, DataType.ExtendedEventIoId));
            ioElementSubItems.Add(_parser.ReadData(2, DataType.ExtendedIoCount));

            var ioCount1BData = _parser.ReadData(2, DataType.ExtendedIoCount1B);
            ioElementSubItems.Add(ioCount1BData);
            for (var j = 0; j < Int32.Parse(ioCount1BData.Value); j++)
            {
                ioElementSubItems.Add(_parser.ReadData(2, DataType.ExtendedIoId1B));
                ioElementSubItems.Add(_parser.ReadData(1, DataType.IoValue1B));
            }

            var ioCount2BData = _parser.ReadData(2, DataType.ExtendedIoCount2B);
            ioElementSubItems.Add(ioCount2BData);
            for (var j = 0; j < Int32.Parse(ioCount2BData.Value); j++)
            {
                ioElementSubItems.Add(_parser.ReadData(2, DataType.ExtendedIoId2B));
                ioElementSubItems.Add(_parser.ReadData(2, DataType.IoValue2B));
            }

            var ioCount4BData = _parser.ReadData(2, DataType.ExtendedIoCount4B);
            ioElementSubItems.Add(ioCount4BData);
            for (var j = 0; j < Int32.Parse(ioCount4BData.Value); j++)
            {
                ioElementSubItems.Add(_parser.ReadData(2, DataType.ExtendedIoId4B));
                ioElementSubItems.Add(_parser.ReadData(4, DataType.IoValue4B));
            }

            var ioCount8BData = _parser.ReadData(2, DataType.ExtendedIoCount8B);
            ioElementSubItems.Add(ioCount8BData);
            for (var j = 0; j < Int32.Parse(ioCount8BData.Value); j++)
            {
                ioElementSubItems.Add(_parser.ReadData(2, DataType.ExtendedIoId8B));
                ioElementSubItems.Add(_parser.ReadData(8, DataType.IoValue8B));
            }

            var ioCountXBData = _parser.ReadData(2, DataType.ExtendedIoCountXB);
            ioElementSubItems.Add(ioCountXBData);

            for (var j = 0; j < Int32.Parse(ioCountXBData.Value); j++)
            {
                ioElementSubItems.Add(_parser.ReadData(2, DataType.ExtendedIoIdXB));
                var extendedElementLength = _parser.ReadData(2, DataType.ExtendedElementLength);
                ioElementSubItems.Add(_parser.ReadData(Convert.ToInt32(extendedElementLength.Value), DataType.ExtendedIoValueXB));
            }
        ...
        private void ParseAndDecodeCodec16Data()
        {
            var avlItems = new List<Data>();
            var gpsElementSubItems = new List<Data>();
            var ioElementSubItems = new List<Data>();
            ...
            ioElementSubItems.Add(_parser.ReadData(2, DataType.EventIoIdCodec16));
            ioElementSubItems.Add(_parser.ReadData(1, DataType.OriginType));
            ...
            for (var j = 0; j < Int32.Parse(ioCount1BData.Value); j++)
            {
                ioElementSubItems.Add(_parser.ReadData(2, DataType.IoId1BCodec16));
                ioElementSubItems.Add(_parser.ReadData(1, DataType.IoValue1B));
            }
            ...
            for (var j = 0; j < Int32.Parse(ioCount2BData.Value); j++)
            {
                ioElementSubItems.Add(_parser.ReadData(2, DataType.IoId2BCodec16));
                ioElementSubItems.Add(_parser.ReadData(2, DataType.IoValue2B));
            }
            ...
            for (var j = 0; j < Int32.Parse(ioCount4BData.Value); j++)
            {
                ioElementSubItems.Add(_parser.ReadData(2, DataType.IoId4BCodec16));
                ioElementSubItems.Add(_parser.ReadData(4, DataType.IoValue4B));
            }
            ...
            for (var j = 0; j < Int32.Parse(ioCount8BData.Value); j++)
            {
                ioElementSubItems.Add(_parser.ReadData(2, DataType.IoId8BCodec16));
                ioElementSubItems.Add(_parser.ReadData(8, DataType.IoValue8B));
            }
